...
K=$4
GPU=$5
TAG=${task}_fewshot_filter

TYPE=prompt-demo

for demos in 1 2 4 8 16
do
  demo_filter_rate=$(echo "scale=5; $demos / $K + 0.0001" | bc)

  dir=result/${TAG}_${K}_${demos}_${demo_filter_rate}
  mkdir -p $dir

  num_demos=$(echo $demos \* $num_labels | bc)
  num_demos=$((num_demos>32 ? 32 : num_demos))

  array_id=0
  for SEED in 13 21 42 87 100
  do
    echo "Start fewshot $TAG $SEED $K $num_demos $demo_filter_rate on $GPU"
