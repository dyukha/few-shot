...
os.chdir("..")
_, task, task_name, num_labels, k, merge_labels, mul, drop_dense, knn_loss, inputs, gpu = sys.argv
num_labels = int(num_labels)
k = int(k)
inputs = eval(inputs)

assert knn_loss in ["mse", "cross_entropy"], knn_loss

model = "knn_linear_fix"
tag = f"{task}_{k}_{num_labels}_{merge_labels}_{drop_dense}_{mul}_fewshot_knn_linear_fix"

input_type = "prompt-demo"

if drop_dense == "drop_dense":
    drop_dense = True
elif drop_dense == "nodrop_dense":
    drop_dense = False
else:
    print(f"Unknown drop_dense type {drop_dense}", file=sys.stderr)
    sys.exit(-1)
