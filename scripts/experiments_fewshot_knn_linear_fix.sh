mkdir -p ../out/fewshot_128_knn_merge_mul_linear_fix_drop_cross
parallel -j4 --lb -n 3 'arr=(na 0 1 2 3); python fewshot_knn_linear_fix.py {1} {2} {3} 128 merge_labels mul drop_dense cross_entropy "[1, 2, 4, 8, 16]" ${arr[{%}]} |& tee ../out/fewshot_128_knn_merge_mul_linear_fix_drop_cross/{1}.txt' ::: \
    SST-2 sst-2 2 \
    cr cr 2 \
    QQP qqp/f1 2 \
    sst-5 sst-5 5 \
...
mkdir -p ../out/fewshot_16_knn_merge_mul_linear_fix_drop_cross
parallel -j4 --lb -n 3 'arr=(na 0 1 2 3); python fewshot_knn_linear_fix.py {1} {2} {3} 16 merge_labels mul drop_dense cross_entropy "[1, 2, 4, 8, 16]" ${arr[{%}]} |& tee ../out/fewshot_16_knn_merge_mul_linear_fix_drop_cross/{1}.txt' ::: \
    SST-2 sst-2 2 \
    cr cr 2 \
    mr mr 2 \
    QQP qqp/f1 2 \
