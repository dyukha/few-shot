...
os.chdir("..")
_, task, task_name, num_labels, k, merge_labels, mul, lm_weight, zero_shot, contrastive, inputs, gpu = sys.argv
...
model = "roberta-large"
tag = f"{task}_{k}_{num_labels}_{merge_labels}_{mul}_fewshot_interpolation_{lm_weight}_{zero_shot}"
...
if zero_shot == "zero":
    ...
    print(f"Unknown shot type {zero_shot}", file=sys.stderr)
    sys.exit(-1)

if contrastive == "contrastive":
    use_contrastive = "--use_contrastive --contrastive_dim 100"
elif contrastive == "nocontrastive":
    use_contrastive = ""
else:
    print(f"Unknown contrastive type {contrastive}", file=sys.stderr)
    sys.exit(-1)
    ...
    for num_demos in inputs:
        ...
        for array_id, seed in enumerate([13, 21, 42, 87, 100]):
            ...
            args = [
                ...
                demo_filter_merge_labels,
                f"--knn_interpolation_model_weight {lm_weight}",
                do_zero_shot,
                use_contrastive,
                # "--knn_linear_fix_drop_dense" if drop_dense else "",
                # f"--knn_linear_fix_loss {knn_loss}",
                "--no_train",
                "--num_sample 1",
