...
tag = f"{task}_{k}_{num_labels}_{merge_labels}_{mul}_fewshot_similar_repeat"
...
def true_parameters(similar, repeat):
    demo_filter_num = similar
    num_demos = similar * repeat
    if mul == "mul":
        ...
        sys.exit(-1)
    directory = f"result/{tag}_{similar}_{repeat}"
    return num_demos, demo_filter_num, directory


def main():
    for similar, repeat in inputs:
        num_demos, demo_filter_num, directory = true_parameters(similar, repeat)
