...
from src.models import BertForPromptFinetuning, RobertaForPromptFinetuning, resize_token_type_embeddings, KnnModel, \
    KnnLinearFixModel, KnnAndRobetaModel
...
class ModelArguments:
    ...
    knn_interpolation_model_weight: float = field(
        default=1.0,
        metadata={"help": "We combine the model distribution (weight w) with the kNN distribution (weight 1 - w)"}
    )
...
class DynamicDataTrainingArguments(DataTrainingArguments):
    ...
    demo_filter_num: int = field(
        default=1,
        metadata={"help": "Only use this number of similar instances in demonstrations"}
    )

    demo_filter_merge_labels: bool = field(
        default=False,
        metadata={"help": "If false, all filtering is done for each label separately"}
    )
    ...
    demo_repeat: int = field(
        default=1,
        metadata={"help": "How many times repeat an element in demonstrations."}
    )

    # demo_filter_type: str = field(
    #     default="similar",
    #     metadata={"help": "'similar' means that we sample from each class demo_filter_rate fraction of demonstraintions that are the closest to the query. "
    #                       "'coresets' means that we find demo_filter_coresets k-centers in the training data and use them for demonstrations"}
    # )

    demo_filter_use_coresets: bool = field(
        default=False,
        metadata={"help": "Whether to reduce available data points to their coresets"}
    )

    demo_filter_coresets: int = field(
        default=1,
        metadata={"help": "The number of coresets for demonstration filtering"}
    )

    demo_filter_coresets_n_similar: int = field(
        default=1,
        metadata={"help": "First few coresets will be the most similar points"}
    )
    ...
    knn_linear_fix_drop_dense: bool = field(
        default=False,
        metadata={"help": "Drop dense layer when doing linear fix for KNN (i.e. use embeddings instead of logits)"}
    )

    knn_linear_fix_loss: str = field(
        default=None,
        metadata={"help": "Loss function for regression for linear fix for KNN. Possible values: 'mse' and 'cross_entropy'"}
    )

    use_contrastive: bool = field(
        default=False,
        metadata={"help": "Whether we use contrastive learning before demonstration filtering"}
    )

    contrastive_dim: int = field(
        default=100,
        metadata={"help": "Dimension for contrastive learning"}
    )

    do_zero_shot: bool = field(
        default=False,
...
def main():
    ...
    use_knn = model_args.model_name_or_path == "knn"
    use_linear_fix = model_args.model_name_or_path == "knn_linear_fix"
    lm_weight = model_args.knn_interpolation_model_weight
    use_interpolation = (1e-9 < lm_weight < 1 - 1e-9)

    print("Template:", data_args.template)
    ...
    data_args.single_sentence_template = data_args.template
    ...
    # Create config
    if use_interpolation or use_knn or use_linear_fix:
        model_name = "roberta-large"
    elif model_args.config_name:
        model_name = model_args.config_name
    else:
    ...
    if use_interpolation:
        model_fn = KnnAndRobetaModel
    elif use_knn:
        model_fn = KnnModel
    elif use_linear_fix:
        model_fn = KnnLinearFixModel
    elif 'prompt' in model_args.few_shot_type:
    ...
    if use_linear_fix or data_args.use_mask_embeddings:
        mask_embedding_training = get_mask_embedding_training(
            ...
            data_args.knn_linear_fix_drop_dense,
            training_args,
            data_args.task_name,
            data_args.num_sample,
            label_word_list
        )
    else:
        mask_embedding_training = None
    ...
    train_dataset = create_dataset("train") if training_args.do_train else None
    eval_dataset = create_dataset("dev") if training_args.do_eval else None
    test_dataset = create_dataset("test") if training_args.do_predict else None
    logger.info("End creating datasets")


    set_seed(training_args.seed)

    logger.info("Start creating the model")
    if use_interpolation:
        model = model_fn(config, lm_weight)
    elif use_knn:
        model = model_fn(config)
    elif use_linear_fix:
        model = model_fn(config, 1, data_args.knn_linear_fix_loss)
    else:
    ...
    model.label_word_list = torch.tensor(label_word_list).long().cuda()
    logger.info("End creating the model")

    # Build metric

    logger.info("Start creating the trainer")
    ...
    trainer = Trainer(
        ...
        compute_metrics=build_compute_metrics_fn(data_args.task_name, data_args.num_sample)
    )
    logger.info("End creating the trainer")
    ...
    if training_args.do_eval:
        ...
        with torch.no_grad():
            ...
            for eval_dataset in eval_datasets:
                trainer.compute_metrics = build_compute_metrics_fn(eval_dataset.args.task_name, data_args.num_sample)
                ...
                if trainer.is_world_master():
                    with open(output_eval_file, "w") as writer:
                        logger.info(f"***** Eval results {eval_dataset.args.task_name} *****")
    ...
    if training_args.do_predict:
        ...
        with torch.no_grad():
            ...
            for test_dataset in test_datasets:
                trainer.compute_metrics = build_compute_metrics_fn(test_dataset.args.task_name, data_args.num_sample)
                ...
                if trainer.is_world_master():
                    ...
                    if training_args.save_logit:
                        ...
                        filename_full = os.path.join(training_args.save_logit_dir, f"{test_dataset.task_name}-{training_args.model_id}-{training_args.array_id}-full.npy")
                        np.save(filename_full, predictions)
                        logits = predictions.reshape([test_dataset.num_sample, -1, num_logits]).mean(axis=0)
                        filename_logits = os.path.join(training_args.save_logit_dir, f"{test_dataset.task_name}-{training_args.model_id}-{training_args.array_id}.npy")
                        np.save(filename_logits, logits)

                test_results.update(test_result)

    # print(f"Total times for inference: {model.total_times}")
...
def build_compute_metrics_fn(task_name: str, num_sample) -> Callable[[EvalPrediction], Dict]:
    def compute_metrics_fn(p: EvalPrediction):
        # def softmax(v):
        #     return torch.nn.functional.softmax(torch.tensor(v), dim=-1).numpy()
        ...
        # lm_weight = 1
        #
        # num_classes = num_logits // 2
        # assert num_classes * 2 == num_logits
        # if lm_weight < 1e-9:
        #     # Only knn
        #     logits = logits_all
        #     # logits_knn = logits_all[:, :num_classes]
        #     # logits_lm = logits_knn
        # elif lm_weight > 1 - 1e-9:
        #     # Only language model
        #     logits = logits_all
        #     # logits_lm = softmax(logits_all[:, :num_classes])
        #     # logits_knn = logits_lm
        # else:
        #     logits_lm = softmax(logits_all[:, :num_classes])
        #     logits_knn = logits_all[:, num_classes:]
        #     logits = logits_lm * lm_weight + logits_knn * (1 - lm_weight)
...
def get_mask_embedding_training(config, cache_dir, drop_dense, training_args, task_name, num_sample, label_word_list):
    ...
    model.label_word_list = label_word_list
    return Trainer(
        model=model,
        args=training_args,
        # compute_metrics=build_compute_metrics_fn(task_name, num_sample)
    )


def get_label_to_word(args, tokenizer):
    if not args.prompt:
        return None, None

    assert args.mapping is not None
    label_to_word = eval(args.mapping)
    ...
    for key in label_to_word:
        ...
        if label_to_word[key][0] not in ['<', '[', '.', ',']:
            ...
            label_to_word[key] = tokenizer._convert_token_to_id(
                tokenizer.tokenize(' ' + label_to_word[key])[0])
        else:
            label_to_word[key] = tokenizer._convert_token_to_id(label_to_word[key])
        word = label_to_word[key]
        logger.info(f"Label {key} to word {tokenizer._convert_id_to_token(word)} ({word})")

    label_list = processors_mapping[args.task_name].get_labels()
    ...
    return label_to_word, label_word_list
